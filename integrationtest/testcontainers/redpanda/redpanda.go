...
func RunContainer(ctx context.Context, opts ...testcontainers.ContainerCustomizer) (*Container, error) {
	...
	// 4. Register network aliases
	for _, alias := range settings.NetworkAliases {
	...
	kafkaPort, err := container.MappedPort(ctx, defaultKafkaAPIPort)
	if err != nil {
...
type KafkaListener struct {
	...
	Address string
	Port    int
	// AdvertisedAddress is the address that shall be used to advertise
	// this listener. Leave blank to use container host ip that is reachable
	// from localhost.
	AdvertisedAddress string
	AdvertisedPort    string
	// AuthenticationMethod is either "none" for plaintext or "sasl" for SASL-SCRAM.
	AuthenticationMethod string
}
